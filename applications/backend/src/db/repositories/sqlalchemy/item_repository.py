...
from src.db.repositories.sqlalchemy.data_models import SQLAlchemyItem
...
class ItemRepository(BaseRepository[SQLAlchemyItem, Item]):
    def __init__(self, async_session: AsyncSession) -> None:
        super().__init__(data_model=SQLAlchemyItem, domain_model=Item, async_seesoon=async_session)
    ...
    async def read_by_user_id(self, user_id: int) -> list[Item]:
        ...
        async with self.async_session.begin():
            items = await self.async_session.execute(
