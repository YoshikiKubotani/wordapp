...
from db.models.sqlalchemy_data_models import SQLAlchemyQuizItem
...
class QuizItemRepository(BaseRepository[SQLAlchemyQuizItem, QuizItem]):
    def __init__(self, async_session: AsyncSession) -> None:
        super().__init__(data_model=SQLAlchemyQuizItem, domain_model=QuizItem, async_seesoon=async_session)
    ...
    async def read_by_quiz_id(self, quiz_id: int) -> list[QuizItem]:
        ...
        async with self.async_session.begin():
            quiz_items = await self.async_session.execute(
                select(self.async_session).where(self.data_model.quiz_id == quiz_id)
            )
            return quiz_items.scalars().all()
