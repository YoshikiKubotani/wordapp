    ...
    def _get_options_for_each_item(
        ...
        for i, target_item_info in enumerate(items):
            ...
            while True:
                ...
                wrong_options, option_source = self._shuffle_and_split(
                    option_source, num_options - 1
                )
                ...
                # 正解の選択肢を追加
                test_item_dict[
                    f"op{correct_option_index+1}"
                ] = target_item_info.japanese
                ...
                for idx, wrong_option_id in enumerate(wrong_options):
                    if idx < correct_option_index:
                        ...
                        test_item_dict[f"op{idx+1}"] = self.item_repository.select_data(
                            {"item_id": wrong_option_id}
                        )[0].japanese
                    if idx >= correct_option_index:
                        logger.debug(f"{idx+2}: {wrong_option_id}")
                        test_item_dict[f"op{idx+2}"] = self.item_repository.select_data(
                            {"item_id": wrong_option_id}
                        )[0].japanese
                break
